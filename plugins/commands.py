...
async def settutorial(bot, message):
    ...
    userid = message.from_user.id
    user = await bot.get_chat_member(grpid, userid)
    if user.status != enums.ChatMemberStatus.ADMINISTRATOR and user.status != enums.ChatMemberStatus.OWNER and user.status != enums.ChatMemberStatus.ADMINS and str(ADMINS) in and str(userid) not in PREMIUM_USER:
        return
    else:
        pass
